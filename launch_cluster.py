...
def main():
    ...
    # Ensure output directories and files exist with headers
    ...
    # Load existing emails to guarantee zero duplicates
    seen_emails = set()
    ...
    # Launch all 10 worker subprocesses
    processes = []
    log_handles = []
    for wid in range(NUM_WORKERS):
        cmd = [
        ...
        log_path = os.path.join(SLICES_DIR, f"worker_{wid}.log")
    ...
    while any(p.poll() is None for _, p in processes):
        # 1. Ingest newly verified rows from each worker's output CSV
        for wid in range(NUM_WORKERS):
            out_f = os.path.join(SLICES_DIR, f"worker_{wid}_verified.csv")
            if os.path.exists(out_f):
                try:
        ...
        if now - last_print >= 4.0:
            ...
            # Check if target reached across all niches
